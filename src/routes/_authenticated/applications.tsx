...
function Applications() {
  ...
  const locations = useMemo(
    () => Array.from(new Set(apps.map((a) => a.location).filter(Boolean) as string[])).sort(),
    [apps],
  );
  ...
  const upsert = useMutation({
    mutationFn: async (a: Partial<App> & { id?: string }) => {
      ...
      if (a.id) {
        ...
        const { error } = await supabase
          .from("applications")
          .insert({ ...a, user_id: u.user.id } as never);
        if (error) throw error;
        saveDraft(null);
      }
    },
  ...
  function exportCSV() {
    ...
    const rows = filtered.map((a) =>
      headers
        .map((h) => JSON.stringify((a as unknown as Record<string, unknown>)[h] ?? ""))
        .join(","),
    );
    ...
    el.href = url;
    el.download = `applications-${Date.now()}.csv`;
    el.click();
  ...
  return (
    <div>
          ...
          {selected.size > 0 && (
            <>
              ...
              <Button
                size="sm"
                variant="destructive"
                onClick={() => softDelete.mutate([...selected])}
              >
      ...
      {showTrash ? (
        <TrashView
          items={trash}
          onRestore={(ids) => restore.mutate(ids)}
          onPurge={(ids) => hardDelete.mutate(ids)}
        />
      ) : (
        <Tabs defaultValue="list">
          ...
          <TabsContent value="board" className="mt-4">
            <BoardView
              items={filtered}
              onStatusChange={(id, status) => updateOne.mutate({ id, patch: { status } })}
              onOpen={setDetailId}
            />
          </TabsContent>
...
function AdvancedFilters({
  ...
  const activeCount =
    ...
    (filter.hasDeadline ? 1 : 0) +
    (filter.hasFollowUp ? 1 : 0) +
    (filter.hasReferral ? 1 : 0) +
    (filter.dateFrom ? 1 : 0) +
    (filter.dateTo ? 1 : 0);
        ...
        <div className="space-y-3 text-sm">
          ...
          <div className="space-y-1.5">
            <label className="flex items-center gap-2">
              <Checkbox
                checked={filter.hasDeadline}
                onCheckedChange={(v) => setFilter({ ...filter, hasDeadline: !!v })}
              />{" "}
              Has deadline
            </label>
            <label className="flex items-center gap-2">
              <Checkbox
                checked={filter.hasFollowUp}
                onCheckedChange={(v) => setFilter({ ...filter, hasFollowUp: !!v })}
              />{" "}
              Has follow-up
            </label>
            <label className="flex items-center gap-2">
              <Checkbox
                checked={filter.hasReferral}
                onCheckedChange={(v) => setFilter({ ...filter, hasReferral: !!v })}
              />{" "}
              Has referral
            </label>
          </div>
    ...
    <Popover>
      ...
      <PopoverContent className="w-[260px]" align="start">
        <div className="text-sm space-y-2">
          {saved.length === 0 ? (
            ...
            saved.map((s) => (
              <div key={s.id} className="flex items-center justify-between gap-2">
                <button
                  className="flex-1 text-left px-2 py-1 rounded hover:bg-accent"
          ...
          <Button size="sm" className="w-full" onClick={onSaveCurrent}>
            Save current filter
          </Button>
        </div>
      </PopoverContent>
                ...
                <tr key={a.id} className="border-t border-border hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3">
                    ...
                    <div className="text-xs text-muted-foreground">
                      <Link
                        to="/companies/$name"
                        params={{ name: encodeURIComponent(a.company) }}
                  ...
                  <td className="px-4 py-3">
                    <Select
                      value={a.status}
                      onValueChange={(v) => onInlineSave(a.id, { status: v })}
                    >
                      <SelectTrigger className={`h-7 w-[130px] text-xs ${STATUS_COLORS[a.status]}`}>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        {STATUSES.map((s) => (
...
function InlineTextCell({
  ...
  return (
    <td
      ...
      onDoubleClick={() => setEditing({ id: app.id, field })}
    >
      {active ? (
        <Input
          autoFocus
          ...
          className="h-7"
          onBlur={(e) => {
            onSave(app.id, { [field]: e.target.value || null } as Partial<App>);
            setEditing(null);
          }}
          onKeyDown={(e) => {
...
function BoardView({
  ...
  const cols = useMemo(
    () => STATUSES.map((s) => ({ status: s, items: items.filter((i) => i.status === s) })),
    [items],
  );
  ...
  function onDragEnd(e: DragEndEvent) {
    ...
    if (!over) return;
    const status = over.startsWith("col:")
      ? over.slice(4)
      : items.find((i) => i.id === over)?.status;
    if (!status) return;
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCorners} onDragEnd={onDragEnd}>
      ...
      >
        {cols.map((c) => (
          <BoardColumn key={c.status} status={c.status} items={c.items} onOpen={onOpen} />
        ))}
      </div>
    </DndContext>
  );
...
function BoardColumn({
  ...
  return (
    <div
      ref={setNodeRef}
      className={`rounded-lg border border-border bg-muted/20 p-2 min-h-[300px] ${isOver ? "ring-2 ring-primary" : ""}`}
    >
      <div className="flex items-center justify-between px-2 py-1.5">
        <span className={`px-2 py-0.5 rounded-md text-xs font-medium ${STATUS_COLORS[status]}`}>
      ...
      <div className="space-y-2 mt-1">
        {items.map((a) => (
          <BoardCard key={a.id} app={a} onOpen={onOpen} />
        ))}
      </div>
    </div>
  );
...
function BoardCard({ app, onOpen }: { app: App; onOpen: (id: string) => void }) {
  ...
  return (
    ...
    >
      ...
      {app.deadline ? (
        <div className="text-[11px] mt-1 text-warning-foreground">
      ...
      <CardContent className="p-3">
        <div className="flex items-center justify-between mb-3">
          <div className="font-semibold flex items-center gap-2">
            <CalendarDays className="size-4" /> {format(cursor, "MMMM yyyy")}
          </div>
        ...
        <div className="grid grid-cols-7 gap-1">
          {days.map((d) => {
            ...
            return (
              <div
                key={d.toISOString()}
                className={`min-h-[92px] rounded-md border border-border p-1.5 ${inMonth ? "bg-background" : "bg-muted/30"}`}
              >
                <div
                  className={`text-[11px] mb-1 ${isSameDay(d, new Date()) ? "font-semibold text-primary" : "text-muted-foreground"}`}
                >
                  {format(d, "d")}
                </div>
          ...
          {events.map((e, i) => (
            <li key={i} className="pl-4 relative">
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                ...
                <span
                  className={`px-1.5 py-0.5 rounded text-[10px] ${STATUS_COLORS[e.app.status]}`}
                >
                  {STATUS_LABELS[e.app.status]}
                </span>
              </div>
              <button
                className="mt-0.5 font-medium hover:underline"
                onClick={() => onOpen(e.app.id)}
              >
                {e.app.role}
              </button>
        ...
        <table className="w-full text-sm">
          <thead className="bg-muted/40 text-xs uppercase tracking-wide text-muted-foreground">
            <tr>
              ...
              <th className="px-4 py-2.5 text-left">Deleted</th>
              <th className="px-4 py-2.5 text-right">Actions</th>
            </tr>
          </thead>
          <tbody>
            {items.map((a) => (
              <tr key={a.id} className="border-t border-border">
                ...
                <td className="px-4 py-3 text-right flex gap-2 justify-end">
                  <Button size="sm" variant="outline" onClick={() => onRestore([a.id])}>
                  ...
                  <Button
                    ...
                    variant="destructive"
                    onClick={() => {
                      if (confirm("Delete permanently?")) onPurge([a.id]);
                    }}
                  >
    ...
    <>
      <SheetHeader>
        ...
        <div className="text-sm text-muted-foreground flex items-center gap-2 flex-wrap">
          <Link
            to="/companies/$name"
            params={{ name: encodeURIComponent(app.company) }}
          ...
          {app.recruiter ? (
            <>
              ...
              <Link
                to="/recruiters/$name"
                params={{ name: encodeURIComponent(app.recruiter) }}
                className="hover:underline flex items-center gap-1"
              >
          ...
          <span>·</span>
          <span className={`px-2 py-0.5 rounded text-xs ${STATUS_COLORS[app.status]}`}>
      ...
      <div className="mt-4 grid gap-4">
          ...
          {app.job_url ? (
            <a href={app.job_url} target="_blank" rel="noreferrer">
              <Button size="sm" variant="outline">
                <ExternalLink className="size-4" /> Job
              </Button>
            </a>
          ) : null}
        </div>
        ...
        <FollowUpEditor
          app={app}
          onSaved={() => qc.invalidateQueries({ queryKey: ["application", appId] })}
        />
...
function FollowUpEditor({ app, onSaved }: { app: App; onSaved: () => void }) {
  ...
  async function save() {
    ...
    const { error } = await supabase
      .from("applications")
      .update({ follow_up_at: iso } as never)
      .eq("id", app.id);
    setSaving(false);
  ...
  return (
    <div className="rounded-lg border border-border p-3">
      <div className="text-sm font-medium mb-2 flex items-center gap-2">
        <Bell className="size-4" /> Follow-up reminder
      </div>
      <div className="flex gap-2">
...
function MarkdownPreview({ text }: { text: string }) {
  ...
  return (
    <div className="prose prose-sm dark:prose-invert max-w-none">
      <Comp remarkPlugins={gfm ? [gfm] : []}>{text || "_Nothing yet._"}</Comp>
    </div>
  );
}
        ...
        <ul className="space-y-1.5">
          {files.map((f) => (
            <li key={f.id} className="flex items-center justify-between text-sm">
              <button
                className="hover:underline truncate max-w-[70%] text-left"
                onClick={() => download(f.file_path, f.name)}
              >
                {f.name}
              </button>
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                ...
                <button
                  ...
                  onClick={() => remove(f.id, f.file_path)}
                >
                  Remove
                </button>
              </div>
...
function ActivityFeed({ appId }: { appId: string }) {
  ...
  return (
    <div className="rounded-lg border border-border p-3">
      <div className="text-sm font-medium mb-2 flex items-center gap-2">
        <Clock className="size-4" /> Activity & status history
      </div>
      {events.length === 0 ? (
            ...
            <li key={e.id} className="pl-3 relative">
              ...
              {e.type === "status_change" && e.from_status && e.to_status ? (
                <div className="mt-1 flex items-center gap-1 text-xs">
                  <span className={`px-1.5 py-0.5 rounded ${STATUS_COLORS[e.from_status]}`}>
                    {STATUS_LABELS[e.from_status]}
                  </span>
                  <span>→</span>
                  <span className={`px-1.5 py-0.5 rounded ${STATUS_COLORS[e.to_status]}`}>
...
function AppDialog({
  ...
  editing: App | null;
  onSubmit: (v: Partial<App>) => void;
  loading: boolean;
}) {
  const [form, setForm] = useState<Partial<App>>(
    () => editing ?? loadDraft() ?? { status: "applied" },
  );
      ...
      >
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            onChange={(e) => setForm({ ...form, application_date: e.target.value || null })}
          />
        </div>
        <div className="space-y-1.5">
