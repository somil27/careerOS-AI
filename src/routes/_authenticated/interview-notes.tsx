...
function InterviewNotes() {
  ...
  const add = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["interview-notes"] });
      setOpen(false);
      setForm({});
      toast.success("Saved");
    },
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Capture questions, feedback, and lessons after every round."
        actions={
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="max-w-2xl">
              <DialogHeader>
                <DialogTitle>New interview note</DialogTitle>
              </DialogHeader>
              <form
              ...
              >
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    placeholder="Phone Screen / Onsite 2"
                  />
                </div>
                <div className="space-y-1.5">
                  <Label>Date</Label>
                  <Input
                    ...
                    onChange={(e) => setForm({ ...form, interview_date: e.target.value || null })}
                  />
                </div>
                <div className="space-y-1.5">
              ...
              <div className="flex items-start justify-between">
                <div>
                  <div className="font-semibold">
                    {n.company}{" "}
                    {n.round && <span className="text-xs text-muted-foreground">· {n.round}</span>}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    {formatDate(n.interview_date ?? n.created_at)}
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  {n.rating && (
                    <span className="inline-flex items-center gap-0.5 text-xs">
                      <Star className="size-3 fill-warning text-warning" /> {n.rating}/5
                    </span>
                  )}
