...
function CompanyProfile() {
  ...
  const totals = apps.reduce<Record<string, number>>((acc, a) => {
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-xs text-muted-foreground flex items-center gap-1">
              <Building2 className="size-3.5" /> Total applications
            </div>
          ...
          <CardContent className="p-4">
            ...
            <div className="flex flex-wrap gap-1">
              {Object.entries(totals).map(([s, n]) => (
                <span key={s} className={`px-2 py-0.5 rounded text-xs ${STATUS_COLORS[s]}`}>
                  {STATUS_LABELS[s]} · {n}
                </span>
              ))}
            </div>
