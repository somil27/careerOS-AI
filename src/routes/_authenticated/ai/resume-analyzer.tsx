...
function AnalyzeTab() {
  ...
  async function run() {
    if (effectiveResume.trim().length < 50)
      return toast.error("Paste your resume or set an active resume (min 50 chars).");
    setLoading(true);
    try {
      const r = await fn({
        data: {
        ...
        <CardContent className="space-y-3">
          {active.data?.name ? (
            <div className="text-xs text-muted-foreground flex items-center gap-1.5">
              <FileText className="size-3.5" /> Active resume:{" "}
              <span className="font-medium text-foreground">{active.data.name}</span>
            </div>
          ) : null}
          ...
          <div className="space-y-1.5">
            ...
            <Textarea
              ...
              placeholder="Paste the JD for tighter analysis…"
            />
          </div>
          <div className="space-y-1.5">
            <Label>
              ...
              {active.data?.text ? (
                <span className="text-xs text-muted-foreground">
                  (leave blank to use active resume)
                </span>
              ) : null}
            </Label>
            <Textarea
      ...
      <div>
        {loading && !result ? (
          ...
          <EmptyState
            ...
            description="Run an analysis to get an ATS score, section-wise breakdown, keyword heatmap, skill gaps, readability, and an improvement checklist."
          />
        ) : (
...
function AnalysisView({ result }: { result: ResumeAnalysis }) {
  const sectionEntries = Object.entries(result.sections) as Array<
    [string, { score: number; findings: string[]; recommendations: string[] }]
  >;
  return (
    <div className="space-y-4">
      ...
      <Card>
        <CardContent className="p-6 flex flex-col sm:flex-row items-start gap-6">
          <div className="flex items-center gap-4">
            <div
              className={`size-24 rounded-full border-4 grid place-items-center font-semibold text-3xl ${scoreTone(result.ats_score)}`}
          ...
          <div className="grid grid-cols-2 gap-3 flex-1 sm:pl-6 sm:border-l">
            <MetricPill
              ...
              suffix={result.readability.grade_level ? ` · ${result.readability.grade_level}` : ""}
            />
            <MetricPill label="Impact" value={result.experience_analysis.impact_score} />
            <MetricPill
              label="Quantified bullets"
              value={result.experience_analysis.quantified_bullets}
              total={result.experience_analysis.total_bullets}
              raw
            />
      ...
      <Tabs defaultValue="overview">
        ...
        <TabsContent value="overview" className="mt-4 grid md:grid-cols-2 gap-4">
          ...
          {result.readability.notes.length ? (
            <Card className="md:col-span-2">
              <CardHeader>
                <CardTitle className="text-sm">Recruiter readability notes</CardTitle>
              </CardHeader>
              <CardContent>
                <ul className="text-sm list-disc pl-5 space-y-1">
                  {result.readability.notes.map((n, i) => (
                    <li key={i}>{n}</li>
                  ))}
                </ul>
              </CardContent>
            ...
            <Card key={name}>
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-sm font-semibold ${scoreTone(s.score)}`}>
                    {Math.round(s.score)}
                  </span>
                </div>
        ...
        <TabsContent value="keywords" className="mt-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-sm">Keyword heatmap</CardTitle>
            </CardHeader>
            <CardContent>
              {result.keyword_heatmap.length === 0 ? (
                ...
                <div className="flex flex-wrap gap-2">
                  {result.keyword_heatmap.map((k, i) => {
                    ...
                    const border = missing
                      ...
                      : `hsl(var(--primary) / ${0.2 + intensity * 0.4})`;
                    return (
                      <span
                        key={i}
                        title={`${k.importance} · ${k.present ? `${k.count} occurrence(s)` : "missing"}`}
                        className="px-2.5 py-1 rounded-md text-xs border flex items-center gap-1.5"
                        style={{ background: bg, borderColor: border }}
                      >
                        <span className="font-medium">{k.keyword}</span>
                        <span className="text-[10px] opacity-70">
                          {missing ? "missing" : `×${k.count}`}
                        </span>
                        {k.importance === "high" ? (
        ...
        <TabsContent value="experience" className="mt-4">
          <AnalysisBlock
            ...
            stats={[
              {
                label: "Impact",
                value: `${Math.round(result.experience_analysis.impact_score)}/100`,
              },
              {
                label: "Quantified",
                value: `${result.experience_analysis.quantified_bullets}/${result.experience_analysis.total_bullets}`,
              },
            ]}
        ...
        <TabsContent value="projects" className="mt-4">
          <AnalysisBlock
            ...
            stats={[{ label: "Count", value: String(result.project_analysis.count) }]}
            findings={result.project_analysis.findings}
            recommendations={result.project_analysis.recommendations}
          />
        </TabsContent>
        <TabsContent value="education" className="mt-4">
          <AnalysisBlock
            title="Education analysis"
            findings={result.education_analysis.findings}
            recommendations={result.education_analysis.recommendations}
          />
        </TabsContent>
        <TabsContent value="achievements" className="mt-4">
          <AnalysisBlock
            ...
            extra={
              result.achievements_analysis.quantified.length ? (
                <div>
                  <div className="text-xs font-semibold uppercase text-muted-foreground mb-1">
                    Quantified metrics found
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {result.achievements_analysis.quantified.map((q, i) => (
                      <span
                        key={i}
                        className="px-2 py-0.5 rounded bg-success/10 text-success text-xs border border-success/20"
                      >
                        {q}
                      </span>
                    ))}
        ...
        <TabsContent value="checklist" className="mt-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-sm">Improvement checklist</CardTitle>
            </CardHeader>
            <CardContent>
...
function ChecklistView({ items }: { items: ResumeAnalysis["improvement_checklist"] }) {
  ...
  if (items.length === 0)
    return <p className="text-sm text-muted-foreground">Nothing to improve — nice work.</p>;
  const groups = items.reduce<Record<string, Array<{ i: number; item: (typeof items)[0] }>>>(
    (acc, item, i) => {
          ...
          <ul className="space-y-1.5">
            {list.map(({ i, item }) => (
              <li key={i} className="flex items-start gap-2.5 text-sm">
                ...
                <div className={done[i] ? "line-through text-muted-foreground" : ""}>
                  {item.item}
                </div>
                <Badge
                  variant={
      ...
      <CardContent className="space-y-3">
        {stats?.length ? (
          <div className="flex gap-2 flex-wrap">
            {stats.map((s) => (
              <div key={s.label} className="px-3 py-1.5 rounded-md bg-muted text-xs">
        ...
        {!findings.length && !recommendations.length && !extra ? (
...
function Chips({
  ...
  title: string;
  items: string[];
  tone?: "success" | "warn" | "info" | "primary";
}) {
    ...
    <Card>
      ...
      <CardContent>
        <ul className="flex flex-wrap gap-1.5">
          {items.map((i, idx) => (
            <li key={idx} className={`px-2.5 py-1 rounded-md text-xs border ${cls}`}>
              {i}
            </li>
          ))}
        </ul>
...
function HistoryTab() {
  ...
  const trend = useMemo(() => {
    ...
    return [...data]
      .reverse()
      .map((d) => ({ id: d.id, score: d.output?.ats_score ?? 0, when: d.created_at }));
  }, [data]);
  ...
  if (!data?.length)
    return (
      <EmptyState
        ...
        description="Run your first analysis to build a score history."
      />
    );
  ...
  return (
    <div className="grid lg:grid-cols-[360px_1fr] gap-4">
          ...
          <div className="divide-y max-h-[520px] overflow-y-auto -mx-2">
            {data.map((row) => (
              <button
                ...
                className={`w-full text-left px-2 py-2.5 hover:bg-accent rounded-md transition-colors ${selected?.id === row.id ? "bg-accent" : ""}`}
              >
                <div className="flex items-center justify-between">
                  <div className="text-sm font-medium truncate">{row.title || "Analysis"}</div>
                  <span
                    className={`text-sm font-semibold ${scoreTone(row.output?.ats_score ?? 0)}`}
                  >
                    {Math.round(row.output?.ats_score ?? 0)}
                  </span>
                </div>
      ...
      <div>
        {selected ? (
          <AnalysisView result={selected.output} />
        ) : (
          <EmptyState
            icon={AlertTriangle}
            title="Pick an analysis"
            description="Select a run on the left to view details."
          />
        )}
      </div>
    </div>
...
function SparkTrend({ points }: { points: number[] }) {
  if (points.length < 2)
    return <div className="text-xs text-muted-foreground">Run more analyses to see a trend.</div>;
  const w = 320,
    h = 60,
    pad = 4;
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-16">
      ...
      {points.map((p, i) => (
        <circle key={i} cx={scaleX(i)} cy={scaleY(p)} r={2.5} fill="hsl(var(--primary))" />
      ))}
    </svg>
  );
}
...
function CompareTab() {
  ...
  const [la, setLa] = useState("Version A");
  const [lb, setLb] = useState("Version B");
  const [role, setRole] = useState("");
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<any>(null);

  async function run() {
    if (a.trim().length < 50 || b.trim().length < 50)
      return toast.error("Paste both resumes (min 50 chars each).");
    setLoading(true);
    try {
      const r = await fn({
        data: {
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">Compare two resume versions</CardTitle>
        </CardHeader>
        <CardContent className="grid md:grid-cols-2 gap-4">
          ...
          {history && history.length >= 2 ? (
            <p className="md:col-span-2 text-xs text-muted-foreground">
              Tip: you have {history.length} past analyses in history — paste the same resume text
              used in those runs to compare directly.
            </p>
          ) : null}
        </CardContent>
      </Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid sm:grid-cols-3 gap-3">
              <div className="p-4 rounded-lg border">
                ...
                <div className={`text-3xl font-semibold ${scoreTone(result.score_a)}`}>
                  {Math.round(result.score_a)}
                </div>
              </div>
              <div className="p-4 rounded-lg border">
                <div className="text-xs text-muted-foreground">{lb}</div>
                <div className={`text-3xl font-semibold ${scoreTone(result.score_b)}`}>
                  {Math.round(result.score_b)}
                </div>
              </div>
              <div className="p-4 rounded-lg border bg-accent">
                <div className="text-xs text-muted-foreground">Winner</div>
                <div className="text-lg font-semibold capitalize">
                  {result.winner === "a" ? la : result.winner === "b" ? lb : "Tie"}
                </div>
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-3">
              <Chips
                title={`Improvements ${la} → ${lb}`}
                items={result.improvements_a_to_b}
                tone="success"
              />
