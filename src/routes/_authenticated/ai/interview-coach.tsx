...
function InterviewCoach() {
  ...
  const [prepForm, setPrepForm] = useState<{
    ...
    use_resume: boolean;
  }>({ company: "", role: "", job_description: "", difficulty: "mixed", use_resume: true });
  ...
  const [practice, setPractice] = useState({
    question: "",
    answer: "",
    kind: "behavioral" as "behavioral" | "technical" | "coding" | "hr",
  ...
  async function runPrep() {
    if (!prepForm.company || !prepForm.role) {
  ...
  async function runPractice() {
    if (!practice.question || !practice.answer) {
      toast.error("Enter question and your answer");
      return;
    }
  ...
  async function runStar() {
    if (!star.question || !star.notes) {
      toast.error("Enter question and your notes");
      return;
    }
    setStarLoading(true);
    try {
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Company research, question banks, STAR generator, and AI-graded mock practice — all persisted."
      />

      <Tabs value={tab} onValueChange={setTab}>
        <TabsList>
          <TabsTrigger value="prep">
            <Building2 className="size-4" /> Prep
          </TabsTrigger>
          <TabsTrigger value="practice">
            <Gauge className="size-4" /> Practice
          </TabsTrigger>
          <TabsTrigger value="star">
            <Wand2 className="size-4" /> STAR
          </TabsTrigger>
          <TabsTrigger value="history">
            <History className="size-4" /> History
            {avgScore != null && (
              <span className={`ml-2 text-xs ${scoreTone(avgScore)}`}>avg {avgScore}</span>
            )}
          </TabsTrigger>
        </TabsList>
          ...
          <Card>
            <CardContent className="p-4 grid md:grid-cols-4 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  value={prepForm.difficulty}
                  onValueChange={(v: any) => setPrepForm({ ...prepForm, difficulty: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="md:col-span-4 flex items-center gap-2 text-xs text-muted-foreground">
                <input
                  id="use-resume"
                  type="checkbox"
                  checked={prepForm.use_resume}
                  onChange={(e) => setPrepForm({ ...prepForm, use_resume: e.target.checked })}
                />
                <label htmlFor="use-resume">
                  Tailor to my active resume
                  {active.data ? ` (${active.data.name})` : " (no active resume)"}
                </label>
              </div>
            </CardContent>
          </Card>
          ...
          {prep && (
            <div className="grid lg:grid-cols-3 gap-4">
                ...
                <CardContent className="space-y-3 text-sm">
                  ...
                  {prep.culture_notes?.length > 0 && (
                    <div>
                      ...
                      <ul className="list-disc pl-5 space-y-1">
                        {prep.culture_notes.map((c: string, i: number) => (
                          <li key={i}>{c}</li>
                        ))}
                      </ul>
                    </div>
                  ...
                  {prep.recent_news?.length > 0 && (
                    <div>
                      ...
                      <ul className="list-disc pl-5 space-y-1">
                        {prep.recent_news.map((c: string, i: number) => (
                          <li key={i}>{c}</li>
                        ))}
                      </ul>
                    </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-2">
                    <Target className="size-4" /> Questions to ask them
                  </CardTitle>
                </CardHeader>
                <CardContent>
              ...
              <QuestionBank
                ...
                items={prep.behavioral}
                kind="behavioral"
                onPractice={loadIntoPractice}
              />
              <QuestionBank
                title="Technical"
                icon={<Code2 className="size-4" />}
                items={prep.technical}
                kind="technical"
                onPractice={loadIntoPractice}
              />
              <QuestionBank
              ...
              <Card className="lg:col-span-3">
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-2">
                    <Code2 className="size-4" /> Coding problems
                  </CardTitle>
                </CardHeader>
                  ...
                  {prep.coding?.map((c: any, i: number) => (
                    <div key={i} className="border rounded-lg p-3 space-y-2">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <Badge className={diffTone(c.difficulty)} variant="outline">
                          {c.difficulty}
                        </Badge>
                      </div>
                      <p className="text-sm text-muted-foreground">{c.prompt}</p>
                      <div className="flex flex-wrap gap-1">
                        {c.topics?.map((t: string, k: number) => (
                      ...
                      <Button
                        ...
                        className="h-7 px-2"
                        onClick={() =>
                          loadIntoPractice(`${c.title}\n\n${c.prompt}`, "coding", c.difficulty)
                        }
                      >
                        <ChevronRight className="size-3" /> Practice
                      </Button>
                    </div>
                  ))}
                </CardContent>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">Coding tips</CardTitle>
                </CardHeader>
                <CardContent>
                  <ul className="list-disc pl-5 space-y-1 text-sm">
                    {prep.coding_tips?.map((t: string, i: number) => (
                      <li key={i}>{t}</li>
                    ))}
                  </ul>
                </CardContent>
              ...
              <Card className="lg:col-span-2">
                <CardHeader>
                  <CardTitle className="text-base text-destructive">Red flags to avoid</CardTitle>
                </CardHeader>
                <CardContent>
                  <ul className="list-disc pl-5 space-y-1 text-sm">
                    {prep.red_flags?.map((t: string, i: number) => (
                      <li key={i}>{t}</li>
                    ))}
                  </ul>
                </CardContent>
          ...
          <Card>
            <CardContent className="p-4 grid md:grid-cols-4 gap-3">
              <div className="space-y-1.5">
                ...
                <Select
                  value={practice.kind}
                  onValueChange={(v: any) => setPractice({ ...practice, kind: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(v: any) => setPractice({ ...practice, difficulty: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  value={practice.role}
                  onChange={(e) => setPractice({ ...practice, role: e.target.value })}
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => setPractice({ ...practice, company: e.target.value })}
                />
              </div>
              <div className="md:col-span-4 space-y-1.5">
                ...
                <Textarea
                  ...
                  onChange={(e) => setPractice({ ...practice, question: e.target.value })}
                  placeholder="Tell me about a time you disagreed with your manager…"
                />
              </div>
              <div className="md:col-span-4 space-y-1.5">
          ...
          <Card>
            <CardContent className="p-4 grid md:grid-cols-2 gap-3">
              <div className="space-y-1.5 md:col-span-2">
                <Label>Behavioral question</Label>
                <Input
                  value={star.question}
                  onChange={(e) => setStar({ ...star, question: e.target.value })}
                  placeholder="Tell me about a time you led a project under a tight deadline."
                />
              </div>
              <div className="space-y-1.5">
                <Label>Role (optional)</Label>
                <Input
                  value={star.role}
                  onChange={(e) => setStar({ ...star, role: e.target.value })}
                />
              </div>
              <div className="space-y-1.5">
                <Label>Company (optional)</Label>
                <Input
                  value={star.company}
                  onChange={(e) => setStar({ ...star, company: e.target.value })}
                />
              </div>
              <div className="md:col-span-2 space-y-1.5">
                <Label>Your raw notes / story</Label>
                <Textarea
                  rows={6}
                  value={star.notes}
                  onChange={(e) => setStar({ ...star, notes: e.target.value })}
                  placeholder="What happened, what you did, numbers if you have them…"
                />
              </div>
              <div className="md:col-span-2">
                <Button onClick={runStar} disabled={starLoading}>
                  {starLoading ? (
                    <Loader2 className="size-4 animate-spin" />
                  ) : (
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {(["situation", "task", "action", "result"] as const).map((k) => (
                <Card key={k}>
                  ...
                  <CardContent>
                    <p className="text-sm leading-relaxed">{starResult[k]}</p>
                  </CardContent>
                </Card>
              ))}
              <Card className="md:col-span-2">
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm">Polished spoken answer</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  <p className="text-sm leading-relaxed whitespace-pre-wrap">
                    {starResult.polished_answer}
                  </p>
                  {starResult.tips?.length > 0 && (
                    <div className="border-t pt-3">
                      ...
                      <ul className="list-disc pl-5 space-y-1 text-sm">
                        {starResult.tips.map((t: string, i: number) => (
                          <li key={i}>{t}</li>
                        ))}
                      </ul>
                    </div>
              ...
              <CardContent className="p-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  {row.kind === "interview_practice" && (
                    <div
                      className={`text-2xl font-semibold ${scoreTone(row.output?.overall_score ?? 0)}`}
                    >
                      {row.output?.overall_score ?? 0}
                    </div>
                  )}
                </div>
...
function QuestionBank({
  ...
  onPractice: (q: string, k: any, d: Difficulty) => void;
}) {
  return (
    <Card>
      <CardHeader>
        ...
        {items?.map((q: any, i: number) => (
          <div key={i} className="border rounded-md p-2.5 space-y-1.5">
            <div className="flex items-start justify-between gap-2">
              ...
              <Badge className={diffTone(q.difficulty)} variant="outline">
                {q.difficulty}
              </Badge>
            </div>
            {q.why_asked && (
            ...
            <Button
              ...
              onClick={() => onPractice(q.question, kind, q.difficulty)}
            >
              <ChevronRight className="size-3" /> Practice
            </Button>
          </div>
  ...
  return (
    <div className="grid md:grid-cols-3 gap-4">
      <Card>
        ...
        <CardContent>
          <div className={`text-5xl font-semibold ${scoreTone(result.overall_score)}`}>
          ...
          {subs.map(([label, val]) => (
            <div key={label}>
              <div className="flex items-center justify-between text-xs mb-1">
                <span>{label}</span>
                <span className={scoreTone(val)}>{val}</span>
              </div>
              <Progress value={val} />
            </div>
          ))}
      ...
      <Card className="md:col-span-3">
        ...
        <CardContent>
          <p className="text-sm leading-relaxed whitespace-pre-wrap">{result.model_answer}</p>
        </CardContent>
      </Card>
    </div>
  );
