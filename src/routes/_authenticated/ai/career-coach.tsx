...
import {
  ...
  listCareerPlans,
  getCareerDashboard,
  adoptRecommendedGoal,
  adoptRecommendedSkill,
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Personalized roadmaps, goal & skill tracking, and tailored recommendations."
      />
...
function OverviewTab({
  ...
  goals: Goal[];
  skills: Skill[];
  onJump: (t: "overview" | "plan" | "goals" | "skills" | "history") => void;
}) {
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          label="Active goals"
          value={dashboard.counts.goals_active}
          icon={<Target className="size-4" />}
        />
        <StatCard
          label="Completed goals"
          value={dashboard.counts.goals_completed}
          icon={<CheckCircle2 className="size-4" />}
        />
        <StatCard
          label="Skills tracked"
          value={dashboard.counts.skills_tracked}
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              <Target className="size-4" /> Goal progress
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="font-medium">{dashboard.goal_progress}%</span>
              </div>
            ...
            {activeGoals.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                No active goals yet.{" "}
                <button className="underline" onClick={() => onJump("goals")}>
                  Add one
                </button>{" "}
                or{" "}
                <button className="underline" onClick={() => onJump("plan")}>
                  generate a plan
                </button>
                .
              </p>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              <Sparkles className="size-4" /> Skill mastery
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="font-medium">{dashboard.skill_progress}%</span>
              </div>
            ...
            {topSkills.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                No skills tracked.{" "}
                <button className="underline" onClick={() => onJump("skills")}>
                  Add a skill
                </button>
                .
              </p>
                ...
                {topSkills.map((s) => (
                  <li key={s.id} className="space-y-1">
                    ...
                    <Progress
                      value={Math.min(
                        100,
                        ((s.current_level ?? 0) / Math.max(1, s.target_level ?? 100)) * 100,
                      )}
                    />
                  </li>
                ))}
            ...
            {dashboard.last_plan_at ? (
              <>
                ...
                <span className="font-medium">
                  {new Date(dashboard.last_plan_at).toLocaleDateString()}
                </span>
                . Refresh it whenever your goals or skills change.
              </>
            ) : (
              "No AI plan yet. Generate your first personalized roadmap."
            )}
          </div>
          <Button onClick={() => onJump("plan")}>
            <Compass className="size-4 mr-1.5" />{" "}
            {dashboard.last_plan_at ? "Refresh plan" : "Generate plan"}
          </Button>
        </CardContent>
      </Card>
    </div>
  ...
  async function run() {
    ...
    try {
      const payload = {
        ...
        resume_text:
          useResume && activeResume.data?.text
            ? activeResume.data.text
            : form.resume_text || undefined,
      };
        ...
        <CardContent className="p-4 space-y-3">
          ...
          <Field label="Your situation (optional)">
            <Textarea
              ...
              placeholder="Bootcamp grad transitioning from marketing…"
            />
          </Field>
          {activeResume.data?.text ? (
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <Card className="lg:col-span-2">
        ...
        <CardContent>
          <p className="text-sm leading-relaxed whitespace-pre-wrap">{result.insights}</p>
        </CardContent>
      </Card>
...
function WeeklyRoadmapCard({ items }: { items: CareerPlan["weekly_roadmap"] }) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <Compass className="size-4" /> Weekly roadmap
        </CardTitle>
      </CardHeader>
...
function MonthlyRoadmapCard({ items }: { items: CareerPlan["monthly_roadmap"] }) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <TrendingUp className="size-4" /> Monthly roadmap
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        {items.map((w, i) => (
...
function RecommendedSkillsCard({
  items,
  onAdopt,
}: {
  items: CareerPlan["skill_recommendations"];
  onAdopt: () => void;
}) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <Sparkles className="size-4" /> Recommended skills
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        {items.map((s, i) => (
          <div key={i} className="border-b border-border last:border-0 pb-3 last:pb-0">
            <div className="flex items-center justify-between gap-2">
              <div className="min-w-0">
                <div className="text-sm font-medium truncate">
                  {s.name}{" "}
                  <span className="text-xs text-muted-foreground font-normal">· {s.category}</span>
                </div>
            ...
            <div className="mt-2 flex items-center gap-2">
              <Progress
                value={Math.min(100, (s.current_level / Math.max(1, s.target_level)) * 100)}
                className="flex-1"
              />
...
function RecommendedGoalsCard({
  items,
  onAdopt,
}: {
  items: CareerPlan["goal_recommendations"];
  onAdopt: () => void;
}) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <Target className="size-4" /> Recommended goals
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        {items.map((g, i) => (
          <div key={i} className="border-b border-border last:border-0 pb-3 last:pb-0">
            ...
            <div className="mt-2 flex items-center justify-between gap-2 text-xs text-muted-foreground">
              <span>
                {g.category} · {g.timeframe_weeks}w
              </span>
              <Button
                size="sm"
                variant="outline"
...
function InternshipsCard({ items }: { items: CareerPlan["internship_recommendations"] }) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <GraduationCap className="size-4" /> Internships & fellowships
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        {items.map((r, i) => (
          <div key={i} className="text-sm">
            <div className="font-medium">
              {r.role} @ {r.company}{" "}
              <span className="text-xs text-muted-foreground font-normal">· {r.type}</span>
            </div>
...
function CompaniesCard({ items }: { items: CareerPlan["company_recommendations"] }) {
  if (!items.length) return null;
  const grouped = {
    reach: [] as typeof items,
    target: [] as typeof items,
    safety: [] as typeof items,
  };
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <Building2 className="size-4" /> Target companies
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
...
function TechCard({ items }: { items: CareerPlan["technology_recommendations"] }) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <Cpu className="size-4" /> Technologies to learn
        </CardTitle>
      </CardHeader>
        ...
        {items.map((t, i) => (
          <div key={i} className="flex items-start justify-between gap-2 text-sm">
            <div className="min-w-0">
              <div className="font-medium">
                {t.name}{" "}
                <span className="text-xs text-muted-foreground font-normal">· {t.category}</span>
              </div>
      ...
      <CardContent className="space-y-2">
        {items.map((c, i) => (
          <div key={i} className="text-sm">
            <div className="font-medium">
              {c.name}{" "}
              <span className="text-xs text-muted-foreground font-normal">· {c.provider}</span>
            </div>
            <div className="text-xs text-muted-foreground">
              {c.difficulty} · {c.duration} · {c.cost_estimate}
            </div>
...
function GoalsTab({ goals, onChange }: { goals: Goal[]; onChange: () => void }) {
  ...
  return (
    <div className="space-y-4">
              ...
              {grouped[s].map((g) => (
                <GoalCard
                  ...
                  onUpdate={async (patch) => {
                    await update({ data: { id: g.id, patch } });
                    onChange();
                  }}
                  onDelete={async () => {
      ...
      {active.length === 0 && (
        <EmptyState label="No goals yet. Add one or generate a plan for AI-recommended goals." />
      )}
    </div>
  );
}
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Category">
              <Input
                ...
                placeholder="Learning, Job Search…"
              />
            </Field>
      ...
      <CardContent className="p-4 space-y-2">
        ...
        <div className="space-y-1">
          ...
          <Slider
            ...
            onValueCommit={(v) =>
              onUpdate({ progress: v[0], status: v[0] >= 100 ? "completed" : goal.status })
            }
          />
        </div>
...
function SkillDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Track a skill</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <Field label="Skill">
            <Input
          ...
          <Field label={`Current level: ${form.current_level}`}>
            <Slider
              ...
              onValueChange={(v) => setForm({ ...form, current_level: v[0] })}
            />
          </Field>
          <Field label={`Target level: ${form.target_level}`}>
            <Slider
              ...
              onValueChange={(v) => setForm({ ...form, target_level: v[0] })}
            />
          </Field>
          <Field label="Notes">
            <Textarea
      ...
      <CardContent className="p-4 space-y-2">
        ...
        <div className="space-y-1">
          ...
          <Slider
            ...
            onValueCommit={(v) =>
              onUpdate({ current_level: v[0], last_practiced_at: new Date().toISOString() })
            }
          />
        </div>
...
function HistoryTab({ plans, onAdopt }: { plans: any[]; onAdopt: () => void }) {
  ...
  if (!plans.length)
    return <EmptyState label="No plans yet. Generate your first plan on the Plan tab." />;
  return (
    <div className="grid gap-4 lg:grid-cols-[280px_1fr]">
            ...
            {plans.map((p) => (
              <li key={p.id}>
                <button
                  ...
                  className={`w-full text-left p-2 rounded-md text-sm hover:bg-muted ${selected?.id === p.id ? "bg-muted" : ""}`}
                >
      ...
      <div>
        {selected ? (
          <PlanResult result={selected.output as CareerPlan} onAdopt={onAdopt} />
        ) : (
          <EmptyState label="Select a plan to view." />
        )}
      </div>
    </div>
  );
