              ...
              <CardContent className="space-y-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <Textarea
                    ...
                    placeholder={
                      activeResume?.text
                        ? "Leaving blank uses your active resume."
                  ...
                  <CardContent className="h-full min-h-[400px] grid place-items-center text-center">
                    <div className="space-y-2 max-w-sm">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Run a match to see fit, skills, salary, interview odds, and a personalized
                        roadmap.
                      </p>
                    </div>
                  </CardContent>
    ...
    const fmt = (n: number) => {
      ...
      try {
        return new Intl.NumberFormat(undefined, {
          style: "currency",
          currency: result.salary_estimate.currency || "USD",
          maximumFractionDigits: 0,
        }).format(n);
      } catch {
        return `${result.salary_estimate.currency} ${n.toLocaleString()}`;
      }
    };
    return {
      min: fmt(result.salary_estimate.min),
      median: fmt(result.salary_estimate.median),
      max: fmt(result.salary_estimate.max),
    };
    ...
    <div className="space-y-4">
      ...
      <div className="grid sm:grid-cols-3 gap-3">
        ...
        <ScoreCard
          ...
          label="Skills match"
          value={result.skills_match.score}
        />
        <ScoreCard
          icon={<Sparkles className="size-4" />}
          label="Tech match"
          value={result.technology_match.score}
        />
      </div>

      {result.verdict ? (
        <Card>
          <CardContent className="pt-4 text-sm">{result.verdict}</CardContent>
        </Card>
      ) : null}
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            ...
            <div className="text-xs text-muted-foreground">
              ...
              {result.salary_estimate.region ? ` · ${result.salary_estimate.region}` : ""}
            </div>
            {result.salary_estimate.notes ? (
              <p className="text-xs text-muted-foreground">{result.salary_estimate.notes}</p>
            ) : null}
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent className="space-y-2">
            ...
            {result.interview_probability.reasoning ? (
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <ChipRow
            ...
            items={result.technology_match.matched}
            tone="success"
          />
          <ChipRow
            title="Partial technologies"
            items={result.technology_match.partial}
            tone="warn"
          />
          <Separator />
                ...
                <li key={i} className="border-l-2 border-primary pl-3">
                  ...
                  {step.outcome ? (
                    <div className="text-xs text-muted-foreground mt-1">
                      <span className="font-medium text-foreground">Outcome:</span> {step.outcome}
                    </div>
                  ) : null}
                </li>
              ))}
...
function HistoryList({
  ...
  items: Array<{ id: string; title: string | null; created_at: string; input: any; output: any }>;
  loading: boolean;
  onOpen: (o: JobMatchResult) => void;
}) {
  if (loading)
    return (
      <Card>
        <CardContent className="pt-6 text-sm text-muted-foreground">Loading history…</CardContent>
      </Card>
    );
  if (!items.length)
    return (
      <Card>
        <CardContent className="pt-6 text-sm text-muted-foreground">
          No past matches yet. Run an analysis to build history.
        </CardContent>
      </Card>
    );
  return (
            ...
            {items.map((it) => {
              ...
              return (
                <li key={it.id} className="py-3 flex items-center gap-3">
                  <div className="size-12 rounded-full border-2 border-primary/40 grid place-items-center text-sm font-semibold tabular-nums">
                    {pct}%
                  </div>
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      ...
                      {it.input?.location ? ` · ${it.input.location}` : ""}
                    </div>
                  </div>
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => out && onOpen(out)}
                    disabled={!out}
                  >
                    Open
                  </Button>
