...
export const interviewPrep = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const system =
      "You are a Principal-level interview coach who has prepped engineers for FAANG, unicorn startups, and top consultancies. Be specific, evidence-based, and opinionated. Output STRICT JSON matching the schema exactly.";
    const diff =
      data.difficulty === "mixed"
        ? "a realistic mix of easy/medium/hard"
        : `weighted toward ${data.difficulty}`;
...
export const generateStarAnswer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const system =
      "You are an executive interview coach. Craft STAR (Situation, Task, Action, Result) answers that are specific, quantified, and 60-90 seconds when spoken. Strict JSON.";
...
export const scoreInterviewAnswer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const system =
      "You are a strict but fair FAANG interview grader. Score honestly on 0-100 for each rubric. Reference specific phrases from the candidate's answer. Strict JSON.";
